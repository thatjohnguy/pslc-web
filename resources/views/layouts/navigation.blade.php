<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <x-nav-link :href="route('member.edit-profile')" :active="request()->routeIs('profile')">
                        {{ __('Profile') }}
                    </x-nav-link>

                    @if(auth()->user()->isAdmin())
                    <x-nav-link :href="route('verify')" :active="request()->routeIs('verify')">
                        <b>{{ __('Verify members') }}</b>
                    </x-nav-link>
                        @endif
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <x-dropdown-link :href="route('member.edit-profile')">
                                {{ __('My profile') }}
                            </x-dropdown-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('member.edit-profile')" :active="request()->routeIs('profile')">
                {{ __('Profile') }}
            </x-responsive-nav-link>

            @if(auth()->user()->isAdmin())
            <x-responsive-nav-link :href="route('verify')" :active="request()->routeIs('verify')">
                <b>{{ __('Verify members') }}</b>
            </x-responsive-nav-link>
            @endif
        </div>
